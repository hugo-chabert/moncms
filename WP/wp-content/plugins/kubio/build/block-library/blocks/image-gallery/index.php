...
class ImageGalleryBlock extends BlockBase {
	...
	public function renderStyle() {
		...
		$id                = LodashBasic::get( $this->block_data, array( 'attrs', 'kubio', 'id' ) );
		...
		$local_style_prefix = "{$style_prefix}local-";
		foreach ( $columns_per_media as $key => $value ) {
			$nr_columns_per_media = $value;

			$style .= $columns_media_sizes[ $key ];
			$style .= "\t.{$local_style_prefix}" . $id . "-container figure.wp-block-kubio-image-gallery-item{\n";
...
class ImageGalleryItemBlock extends BlockBase {
	...
	public function computed() {
		$this->parent_block = Registry::getInstance()->getLastBlockOfName( 'kubio/image-gallery' );
