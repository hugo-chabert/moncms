...
class VideoBlock extends BlockBase {
	...
	function kubio_video_shortcode( $atts ) {
		...
		if ( $atts['type'] === 'external' ) {
			$this->kubio_html_embed_iframe( $atts['url'], $atts['autoplay'] );
		} else {
			$this->kubio_html_embed_video( $atts['url'], $atts['attributes'] );
		}
		$content = ob_get_clean();
		return $content;
	}

	function kubio_html_embed_iframe( $url, $autoplay ) {
		$iframe_html = '<iframe src=' . esc_url( $url ) . " class='h-video-main'" . ( ( $autoplay == '0' ) ? 'allow="autoplay"' : '' ) . '  allowfullscreen></iframe>';
		echo $iframe_html;
	}

	function kubio_html_embed_video( $url, $attributes ) {
		$poster_url  = $this->getAttribute( 'posterImage.url' );
		$poster_attr = $poster_url ? "poster='" . esc_url( $poster_url ) . "' " : '';
		$video_html  = "<video class='h-video-main' playsinline {$poster_attr} {$attributes}><source src='{$url}' type='video/mp4' /></video>";
		echo $video_html;
	}
}
