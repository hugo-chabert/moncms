...
class StyleParser {
	...
	protected function __construct() {
		 $properties = array(
			 ...
			 new UnitValuePercentage( 'maxHeight' ),
			 new UnitValuePercentage( 'minHeight' ),
	...
	public function transform( $obj, $context, $skipNormal = false ) {
		...
		foreach ( $obj as $prop => $___ ) {
			if ( isset( $this->groups[ $prop ] ) ) {
				if ( LodashBasic::isString( $obj[ $prop ] ) ) {
					$css = LodashBasic::merge( $css, $this->evaluate( array( 'prop' => $obj[ $prop ] ) ) );
				} else {
