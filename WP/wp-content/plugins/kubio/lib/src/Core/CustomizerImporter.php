...
class CustomizerImporter {
	...
	private function postProcessBlock( $parsed_block, $current_data ) {
		...
		if ( $block_name === 'kubio/query-loop' ) {
			...
			Arr::set(
				$parsed_block,
				'attrs.kubio.props.layout.itemsPerRow',
				intval( $items_per_row )
			);
		}
	...
	private function normalizeLogo( $parsed_block, $data ) {
		...
		$curernt_data = $this->getCurrentData();
		if ( $alternate_logo = Arr::get( $curernt_data, 'alternate_logo', false ) ) {
	...
	public static function themeHasModifiedOptions() {
		...
		foreach ( $keys as $key ) {
			if ( strpos( $key, 'header.' ) === 0 || strpos( $key, 'front-header.' ) === 0 ) {
				return true;
			}
		}
