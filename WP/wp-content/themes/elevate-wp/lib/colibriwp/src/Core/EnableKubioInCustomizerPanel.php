...
class EnableKubioInCustomizerPanel extends \WP_Customize_Panel {
    ...
    protected function render() {
        ...
        $message = __( 'The Kubio plugin takes Elevate to a whole new level by adding new and powerful editing and styling options. Wanna have full control over your design with Elevate?', 'kubio' );
        ...
        <?php if ( $this->getPluginState( $this->plugin_slug ) !== PluginsManager::ACTIVE_PLUGIN ) : ?>
            ...
            <?php if ( $this->getPluginState( $this->plugin_slug ) === PluginsManager::NOT_INSTALLED_PLUGIN ) : ?>
                <button data-colibri-plugin-action="install" class="button button-primary kubio-open-editor-panel-button ">
                    <span data-action="install"><?php esc_html_e( 'Install Kubio', 'kubio' ); ?></span>
                </button>
            <?php elseif ( $this->getPluginState( $this->plugin_slug ) === PluginsManager::INSTALLED_PLUGIN ) : ?>
                <button data-colibri-plugin-action="activate" class="button button-primary kubio-open-editor-panel-button">
                    <span data-action="activate"><?php esc_html_e( 'Activate Kubio', 'kubio' ); ?></span>
                </button>
            <?php endif; ?>

                <script>
                    (function () {
                        document.querySelector('.kubio-open-editor-panel-button').addEventListener('click', function (e) {
                            const action = e.target.dataset.action
                            console.log({action})
                            if(action === 'install'){
                                window.location = '<?php echo esc_url( $this->getInstallLink() ); ?>';
                            }
                            else if (action === 'activate'){
                                window.location = '<?php echo esc_url( $this->getActivationLink($this->plugin_slug . $this->plugin_version) ); ?>';
                            }
                        })
                    })();
                </script>
